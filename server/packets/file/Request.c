    ...
    if (op->type == CUOT_FILE_REQUEST)
    {
        ...
        switch (aio_error(p))
        {
            case EINPROGRESS:
                ...
                break;
            case ECANCELED:
                CRPFailureSend(user->crp, op->session, ECANCELED, "操作被取消");
                break;
            case 0:

                break;
            default:
                CRPFailureSend(user->crp, op->session, EIO, "操作失败");
                break;
        }
        ssize_t ioRet = aio_return(&opData->aio);
        if (ioRet == 0)
        {
            CRPFileDataEndSend(user->crp, op->session, FEC_OK);
            close(opData->aio.aio_fildes);
            free((void *) opData->aio.aio_buf);
            free(opData);
            op->onCancel = NULL;
        ...
        {
            CRPFileDataSend(user->crp, op->session, (CRP_LENGTH_TYPE) ioRet, opData->seq++, (void *) p->aio_buf);
...
int ProcessPacketFileRequest(POnlineUser user, uint32_t session, CRPPacketFileRequest *packet)
{
    if (packet->type == FST_SHARED)
    {
        if (DataFileExist(packet->key))
        {
            ...
            if (fd < 0)
            {
                CRPFailureSend(user->crp, session, ENOENT, "无法读取文件");
                goto fail;
            }
            struct stat fileInfo;
            if (fstat(fd, &fileInfo) == -1)
            {
                CRPFailureSend(user->crp, session, EIO, "无法获得文件状态");
                goto fail;
            }
            opData = (PUserOperationFileRequest) calloc(1, sizeof(UserOperationFileRequest));
            if (opData == NULL)
            {
                CRPFailureSend(user->crp, session, ENOMEM, "无法创建文件请求操作");
                goto fail;
            }
            opData->seq = 0;
            ...
            {
                CRPFailureSend(user->crp, session, ENOMEM, "无法分配缓冲区");
                goto fail;
            }
            ...
            if (op == NULL)
            {
                CRPFailureSend(user->crp, session, EMFILE, "无法创建用户操作");
                goto fail;
            }
        ...
        {
            CRPFailureSend(user->crp, session, ENOENT, "文件未找到");
        }
    }
    else
    {
        ...
        {
            CRPFailureSend(user->crp, session, ENOSYS, "不支持的文件类型");
        }
    }
    return 1;
}