...
int CRPLoginLoginSend(CRPContext context, uint32_t sessionID, const char *username, const unsigned char *password)
{
    ...
    ssize_t ret = CRPSend(context, CRP_PACKET_LOGIN_LOGIN, sessionID, packet, sizeof(CRPPacketLogin) + n);
    free(packet);
    return ret != -1;
}